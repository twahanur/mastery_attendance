...
// Helper function to create dynamic validation schema
async function createDynamicAuthSchema(includeFields: string[]) {
  const schema: any = {};

  if (includeFields.includes('email')) {
    schema.email = Joi.string().email().required().messages({
  ...
  if (includeFields.includes('username')) {
    schema.username = await validationService.getUsernameSchema();
  }

  if (includeFields.includes('password')) {
    schema.password = await validationService.getPasswordSchema();
  }

  if (includeFields.includes('firstName')) {
    schema.firstName = Joi.string().min(2).max(50)[includeFields.includes('firstNameRequired') ? 'required' : 'optional']().messages({
  ...
  if (includeFields.includes('lastName')) {
    schema.lastName = Joi.string().min(2).max(50)[includeFields.includes('lastNameRequired') ? 'required' : 'optional']().messages({
...
// Auth validation schemas - now using lazy schemas for dynamic validation
...
export const getRegisterSchema = async () => {
  return createDynamicAuthSchema(['email', 'username', 'password', 'firstName', 'lastName']);
};

export const getCreateEmployeeSchema = async () => {
  const baseSchema = await createDynamicAuthSchema(['email', 'username', 'password', 'firstName', 'firstNameRequired', 'lastName', 'lastNameRequired']);
...
// For backward compatibility, export static versions
...
export const getChangePasswordSchema = async () => {
  return Joi.object({
    currentPassword: Joi.string().required().messages({
      'any.required': 'Current password is required'
    }),
    newPassword: await validationService.getPasswordSchema()
  });
};

// For backward compatibility - static version (fallback)
